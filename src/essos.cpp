...
bool EssContextInit( EssCtx *ctx )
{
   ...
   if ( ctx )
   {
      ...
      if ( ctx->autoMode )
      {
         // Auto-detect mode based on actual support capabilities
         bool waylandAvailable = EssContextSupportWayland(ctx);
         bool directAvailable = EssContextSupportDirect(ctx);
         bool waylandRequested = ((getenv("WAYLAND_DISPLAY") != 0) ? true : false);
         
         if ( waylandAvailable && waylandRequested )
         {
            ctx->isWayland = true;
         }
         else if ( directAvailable )
         {
            ctx->isWayland = false;  // Prefer Direct mode when Wayland not available
         }
         else if ( waylandAvailable )
         {
            ctx->isWayland = true;   // Fall back to Wayland if Direct not available
         }
         else
         {
            sprintf( ctx->lastErrorDetail, "Error: Neither Wayland nor Direct mode is available" );
            pthread_mutex_unlock( &ctx->mutex );
            goto exit;
         }
         
         INFO("auto set mode: isWayland %d (wayland available: %d, direct available: %d)", 
              ctx->isWayland, waylandAvailable, directAvailable);
      }
...
#else
// Stub shell surface functions for non-Wayland builds
static void essShellSurfacePing(void *data, struct wl_shell_surface *shell_surface, uint32_t serial)
{
   ESS_UNUSED(data);
   ESS_UNUSED(shell_surface);
